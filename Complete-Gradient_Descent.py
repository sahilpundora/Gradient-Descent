...
print 'Check that targetNorm values are between 0 and 1, minimum value is ', min(targetNorm), 'maximum is ', max(
    targetNorm)
...
print 'Number values assigned to test is', len(test), ' and % assigned to test is ', (len(test) / float(
    len(fullTable))) * 100, '%'

print 'Number values assigned to train is', len(train), ' and % assigned to train is ', (len(train) / float(
    len(fullTable))) * 100, '%'
